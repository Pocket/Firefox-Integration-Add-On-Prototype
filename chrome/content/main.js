...
var pktUI = (function() {
	...
	var savePanelHeights = {collapsed: 149, expanded: 268};
    ...
    function pocketPanelDidShow(event) {
        getButton().setAttribute('open', 'true');
    ...
    function pocketPanelDidHide(event) {
        getButton().removeAttribute('open');
    ...
    function showSignUp() {
        getFirefoxAccountSignedInUser(function(userdata)
        {
            ...
            var panelId = showPanel("chrome://pocket/content/panels/signup.html?pockethost=" + Services.prefs.getCharPref("browser.pocket.site") + "&fxasignedin=" + fxasignedin + "&variant=" + variant + '&inoverflowmenu=' + inOverflowMenu + "&locale=" + getUILocale(), {
            		onShow: function() {
                    },
        			onHide: panelDidHide,
    ...
    function saveAndShowConfirmation(url, title) {
        ...
        // Save the url to the current selected browser to track
        if (isValidURL) {
            var selectedBrowser = gBrowser.selectedBrowser
            selectedBrowser.pkt_savedURL = url;

            var button = getButton();
            button.classList.add('pkt_added');

            // Show the Pocket into bookmark list animation
            //setTimeout(function(){showPocketAnimation();},1);
        }
    	...
    	var panelId = showPanel("chrome://pocket/content/panels/saved.html?pockethost=" + Services.prefs.getCharPref("browser.pocket.site") + "&premiumStatus=" + (pktApi.isPremiumUser() ? '1' : '0') + '&inoverflowmenu='+inOverflowMenu + "&locale=" + getUILocale(), {
    		onShow: function() {
                ...
                var options = {
                    ...
                    error: function(error, request) {
                        ...
                        delete selectedBrowser.pkt_savedURL
                        button.classList.remove('pkt_added');
    ...
    function showPanel(url, options) {
    	...
    	panel.openPopup(getButton(), "bottomcenter topright", 0, 0, false, false);
        return _panelId;
    }
    ...
    function resizePanel(options) {
    	...
    	setTimeout(function(){
	    	...
	    	getPanelFrame().style.height = document.getElementById('pocket-panel-container').clientHeight + 'px';
	    },1);
    }
    ...
    function tabAttrModified(event) {
        ...
        var pktAddedCSSClass = 'pkt_added';

        // Get browser for tab changed
        var browser = gBrowser.getBrowserForTab(event.target);

        // Don't do anything if the browser is not the current browser
        if (gBrowser.selectedBrowser !== browser) { return; }

        // Check if we have to remove the
        var button = getButton();
        var pkt_savedURL = browser.pkt_savedURL;
        if (typeof pkt_savedURL === 'undefined') {
            button.classList.remove(pktAddedCSSClass);
            return;
        }

        // Update the button for the browser in case the URL changed
        var url = browser.currentURI.spec;
        if (url === pkt_savedURL) {
            button.classList.add(pktAddedCSSClass);
            return;
        }

        button.classList.remove(pktAddedCSSClass);
        delete browser.pkt_savedURL;
    }

    // Add Events for tab events
    var tabContainer = gBrowser.tabContainer;
    tabContainer.addEventListener("TabAttrModified", tabAttrModified, false);
    ...
    function getButton() {
    	return document.getElementById('pocket-menu-button');
    }

    function getPanel() {
    	return document.getElementById('pocket-panel');
    }
    ...
    function isInOverflowMenu() {
    	...
    	// Nick, toggle true/false here for testing:
    	return false;
    }
